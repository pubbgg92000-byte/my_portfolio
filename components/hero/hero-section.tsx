...
export function HeroSection() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo(
        ...
        { autoAlpha: 1, y: 0, scale: 1, duration: 0.85, ease: "power3.out", stagger: 0.08 },
      );
      gsap.fromTo(
        ".hero-particle",
        { autoAlpha: 0, scale: 0.4 },
        { autoAlpha: 1, scale: 1, duration: 1.2, ease: "power2.out", stagger: 0.04, delay: 0.25 },
      );
    }, root);
  ...
  return (
    <section ref={rootRef} className="relative isolate min-h-screen overflow-hidden bg-[#06070B] text-white">
      <HeroBackground />
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-[96rem] flex-col px-5 sm:px-8">
        ...
        <div className="grid flex-1 gap-8 py-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center 2xl:gap-10">
          ...
          <div className="hero-reveal relative min-h-[28rem] overflow-hidden rounded-[2rem] border border-white/10 bg-black/32 shadow-frame backdrop-blur-xl">
            <div className="absolute inset-6 rounded-[1.5rem] border border-cyan-200/10" />
            <div className="absolute inset-x-8 top-12 h-px bg-cyan-200/20" />
            <div className="absolute inset-y-12 right-12 w-px bg-cyan-200/20" />
            <div className="absolute bottom-10 left-10 right-10 h-px bg-cyan-200/20" />
            ...
            <div className="hero-reveal mx-auto flex w-max items-center gap-3 rounded-full border border-white/10 bg-black/45 px-5 py-3 text-xs font-semibold uppercase tracking-[0.18em] text-slate-200 shadow-frame backdrop-blur-xl">
  ...
  return (
    <div className="pointer-events-none absolute inset-0 -z-10">
      ...
      {Array.from({ length: 24 }, (_, index) => (
        <span
          key={index}
          className="hero-particle absolute h-1 w-1 rounded-full bg-cyan-100/70 shadow-[0_0_18px_rgb(34_211_238/0.7)]"
          style={{
            left: `${(index * 37) % 100}%`,
            top: `${8 + ((index * 23) % 82)}%`,
